...
export const localStorageManager = {
  ...
  deleteFilme(id: string) {
    // Primeiro, excluir sessões associadas a este filme
    let sessoes = this.getSessoes(); //
    const sessoesFiltradas = sessoes.filter((sessao) => sessao.filmeId !== id);
    if (sessoesFiltradas.length !== sessoes.length) {
      saveAll('sessao', sessoesFiltradas); // Salva a lista de sessões atualizada //
      // console.log(`Sessões associadas ao filme ${id} foram excluídas.`);
    }

    // Então, excluir o filme
    let filmes = this.getFilmes(); //
  ...
  deleteSala(id: string) { //
    // Adicionar lógica para excluir sessões associadas a esta sala, se necessário no futuro
    let salas = this.getSalas(); //
  ...
  deleteSessao(id: string) { //
    // Adicionar lógica para excluir ingressos associados a esta sessão, se necessário no futuro
    let sessoes = this.getSessoes(); //
